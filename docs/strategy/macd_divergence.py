def generate_macd_dive_signal(df, hist_upper=60, hist_lower=-200, 
    ...
    # 현재 히스토그램 값
    current_hist = df['hist_direction_dive'].iloc[-1]
    ...
    # 히스토그램 방향
    hist_direction = current_hist - df['hist_direction_dive'].iloc[-2]
    ...
    if hist_direction > 0 and price_change_pct < -price_threshold:
        ...
        for i in range(lookback):
            ...
            if prev_idx >= 0:  # 인덱스가 유효한지 확인
                hist_diff = df['hist_direction_dive'].iloc[current_idx] - df['hist_direction_dive'].iloc[prev_idx]
